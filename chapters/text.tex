...
Importantly, this is language-agnostic, and \package{biblatex} will make use of \package{polyglossia} (the \hologo{LuaLaTeX} replacement for \package{babel}) and \package{csquotes}.
Therefore, through changing the desired document language for just these packages, all others including \package{biblatex} will quickly adjust automatically.
...
% Don't ever use macros in sectional commands etc.
% They will be written to auxiliary files, where Unicode and macro expansions might come to haunt you.
% Further, they will form the PDF bookmarks, which only accept a very basic character set.
% The first problem will always be an issue, but might work in your case.
% The second one could be solved using \texorpdfstring
